...
## 位置調整してsummon
execute if data storage : _[-1].hologram_ui_light[-1].text rotated ~-60 ~ positioned ~ ~2 ~ run function hologram_ui_light:summon/hologram/adjust/yaw

data remove storage : _[-1].hologram_ui_light[-1]
execute if data storage : _[-1].hologram_ui_light[-1] run function hologram_ui_light:summon/hologram/
